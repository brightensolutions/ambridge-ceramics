...
// ============================================================
// MODEL COMPONENT WITH FULL LOGGING
// ============================================================
function Model({
  url,
  crownType,
  ...
  // Helper to clone and assign material
  const applyMaterial = (node: THREE.Mesh, material: THREE.Material | null) => {
    if (!material) return;
  ...
  // Clone and process on every change
  useEffect(() => {
    if (!scene) return;

    console.log("🔄 Re-cloning scene...");
    console.log("   hideCrown =", hideCrown);
    console.log("   hideAbutment =", hideAbutment);
    console.log("   crownMaterial =", crownType);
    console.log("   abutmentMaterial =", abutmentType);
    ...
    let crownMeshFound = false;
    let abutmentMeshFound = false;

    clone.traverse((node: any) => {
      if (!node.isMesh) return;
      const name = node.name;

      // Crown detection (adjust if needed)
      if (name === "crown_contour_Mesh.001" || name.toLowerCase().includes("crown")) {
        crownMeshFound = true;
        console.log(`👑 Crown mesh identified: ${name}`);
        if (crownMaterial) applyMaterial(node, crownMaterial);
        node.visible = !hideCrown;
        console.log(`   → visible set to ${!hideCrown}`);
      }
      // Abutment detection
      else if (name === "Titanium_Abutment_to_fit_Mesh.001" || name.toLowerCase().includes("abutment")) {
        abutmentMeshFound = true;
        console.log(`🔩 Abutment mesh identified: ${name}`);
        if (abutmentMaterial) applyMaterial(node, abutmentMaterial);
        node.visible = !hideAbutment;
        console.log(`   → visible set to ${!hideAbutment}`);
      }
      // Planes
      else if (name === "Plane.001" || name === "Plane") {
        node.visible = false;
        console.log(`✈️ Hidden plane: ${name}`);
      }
    ...
    if (!crownMeshFound) console.warn("⚠️ No crown mesh found! Check mesh names.");
    if (!abutmentMeshFound) console.warn("⚠️ No abutment mesh found! Check mesh names.");
    ...
    setModel(clone);
    console.log("✅ New model set in state\n");
  }, [scene, crownMaterial, abutmentMaterial, hideCrown, hideAbutment, crownType, abutmentType]);
...
// ============================================================
// PRODUCT DATA (full, same as before)
// ============================================================
...
export default function ProductDetailPage({
  ...
  // Log button clicks
  useEffect(() => {
    console.log("🔘 State changed: hideCrown =", hideCrown, "hideAbutment =", hideAbutment);
  }, [hideCrown, hideAbutment]);
    ...
    <main className="bg-white min-h-screen font-sans">
      ...
      <section className="pt-40 pb-20 px-6 lg:px-12 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-b from-gray-50 to-[#a2d8b2]/20 rounded-[2rem] h-[550px] border border-[#a2d8b2]/30 overflow-hidden relative shadow-sm group">
              ...
              {isClient && !modelError && (
                <div className="absolute bottom-6 right-6 flex flex-col gap-3 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <button onClick={() => handleZoom("in")} className="bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg border border-gray-100 hover:bg-[#a2d8b2] hover:text-teal-950 transition-all duration-300 text-gray-600 hover:-translate-y-1" title="Zoom In">
                    <ZoomIn className="w-5 h-5" />
                  </button>
                  <button onClick={() => handleZoom("out")} className="bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg border border-gray-100 hover:bg-[#a2d8b2] hover:text-teal-950 transition-all duration-300 text-gray-600 hover:-translate-y-1" title="Zoom Out">
                    <ZoomOut className="w-5 h-5" />
                  </button>
                  <button onClick={handleReset} className="bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg border border-gray-100 hover:bg-gray-100 transition-all duration-300 text-gray-600 mt-2 hover:-translate-y-1" title="Reset View">
              ...
              {isClient && modelError && (
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <button onClick={() => setModelError(false)} className="px-6 py-2 bg-[#a2d8b2] text-gray-900 font-medium rounded-full hover:bg-[#8ec29e] transition-colors shadow-sm">Try Again</button>
                  </div>
                </div>
              )}
              {!isClient && (
            ...
            <div className="flex flex-col gap-6 bg-gray-50/50 p-6 rounded-[2rem] border border-[#a2d8b2]/20">
              {!hideCrown && (
                <div>
                  <div className="flex justify-between items-center mb-3 ml-1">
                    ...
                    <button onClick={() => { console.log("👆 Hide Crown clicked"); setHideCrown(true); }} className="text-xs px-2 py-1 bg-red-50 text-red-500 hover:bg-red-100 rounded-full transition-colors">Hide Crown</button>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    {["Zirconia", "LISI", "PFM"].map((item) => (
                      <button key={item} onClick={() => { console.log(`👑 Crown material changed to ${item}`); setSelectedCrown(item); }} className={`px-4 py-2.5 rounded-full border transition-all duration-300 font-medium ${selectedCrown === item ? "bg-[#a2d8b2] text-gray-900 border-[#a2d8b2] shadow-md shadow-[#a2d8b2]/30" : "bg-white text-gray-600 border-gray-200 hover:border-[#a2d8b2] hover:bg-[#a2d8b2]/10"}`}>
                        {item}
                      </button>
                    ))}
                  </div>
              ...
              {hideCrown && (
                <div className="bg-red-50 rounded-xl p-4 border border-red-200">
                  <div className="flex justify-between items-center">
                    <div><h3 className="text-sm font-bold uppercase tracking-widest text-red-500">Crown Hidden</h3><p className="text-xs text-gray-500 mt-1">Crown is currently hidden in 3D model</p></div>
                    <button onClick={() => { console.log("👆 Unhide Crown clicked"); setHideCrown(false); }} className="text-sm px-4 py-2 bg-[#a2d8b2] text-gray-900 hover:bg-[#8ec29e] rounded-full transition-colors font-medium">Unhide Crown</button>
                  </div>
                </div>
              )}
              {!hideAbutment && (
                <div>
                  <div className="flex justify-between items-center mb-3 ml-1">
                    ...
                    <button onClick={() => { console.log("👆 Hide Abutment clicked"); setHideAbutment(true); }} className="text-xs px-2 py-1 bg-red-50 text-red-500 hover:bg-red-100 rounded-full transition-colors">Hide Abutment</button>
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    {["Titanium", "Zirconia", "Anodised"].map((item) => (
                      <button key={item} onClick={() => { console.log(`🔩 Abutment material changed to ${item}`); setSelectedAbutment(item); }} className={`px-4 py-2.5 rounded-full border transition-all duration-300 font-medium ${selectedAbutment === item ? "bg-[#a2d8b2] text-gray-900 border-[#a2d8b2] shadow-md shadow-[#a2d8b2]/30" : "bg-white text-gray-600 border-gray-200 hover:border-[#a2d8b2] hover:bg-[#a2d8b2]/10"}`}>
                        {item}
                      </button>
                    ))}
                  </div>
              ...
              {hideAbutment && (
                <div className="bg-red-50 rounded-xl p-4 border border-red-200">
                  <div className="flex justify-between items-center">
                    <div><h3 className="text-sm font-bold uppercase tracking-widest text-red-500">Abutment Hidden</h3><p className="text-xs text-gray-500 mt-1">Abutment is currently hidden in 3D model</p></div>
                    <button onClick={() => { console.log("👆 Unhide Abutment clicked"); setHideAbutment(false); }} className="text-sm px-4 py-2 bg-[#a2d8b2] text-gray-900 hover:bg-[#8ec29e] rounded-full transition-colors font-medium">Unhide Abutment</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* RIGHT COLUMN - Product Information (unchanged) */}
          <div className="space-y-6">
            <div className="hidden lg:block">
              <span className="text-sm font-bold uppercase tracking-widest text-[#7ab88a] bg-[#a2d8b2]/20 px-3 py-1 rounded-full">{product.category}</span>
            ...
            <div className="space-y-3">
              ...
              <div className={`border rounded-xl overflow-hidden transition-all duration-300 ${open === "mat" ? "border-[#a2d8b2] shadow-md shadow-[#a2d8b2]/20" : "border-gray-100 shadow-sm hover:border-[#a2d8b2]/50"}`}>
                <button onClick={() => toggle("mat")} className={`w-full flex justify-between items-center px-4 py-3.5 text-left transition-colors ${open === "mat" ? "bg-[#a2d8b2]/10" : "bg-white hover:bg-[#a2d8b2]/5"}`}>
                  ...
                  <div className={`p-1.5 rounded-full transition-all duration-300 ${open === "mat" ? "bg-[#a2d8b2] text-gray-900 rotate-180" : "bg-gray-50 text-gray-400"}`}><ChevronDown className="w-4 h-4" /></div>
                </button>
              ...
              <div className={`border rounded-xl overflow-hidden transition-all duration-300 ${open === "ben" ? "border-[#a2d8b2] shadow-md shadow-[#a2d8b2]/20" : "border-gray-100 shadow-sm hover:border-[#a2d8b2]/50"}`}>
                <button onClick={() => toggle("ben")} className={`w-full flex justify-between items-center px-4 py-3.5 text-left transition-colors ${open === "ben" ? "bg-[#a2d8b2]/10" : "bg-white hover:bg-[#a2d8b2]/5"}`}>
                  ...
                  <div className={`p-1.5 rounded-full transition-all duration-300 ${open === "ben" ? "bg-[#a2d8b2] text-gray-900 rotate-180" : "bg-gray-50 text-gray-400"}`}><ChevronDown className="w-4 h-4" /></div>
                </button>
                <div className={`grid transition-all duration-300 ease-in-out ${open === "ben" ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                  <div className="overflow-hidden bg-white"><div className="p-4 pt-3 border-t border-[#a2d8b2]/20"><ul className="space-y-3">{product.benefits.map((b: string, i: number) => (<li key={i} className="flex gap-3 items-start text-sm text-gray-600"><span className="w-1.5 h-1.5 rounded-full bg-[#a2d8b2] mt-1.5 flex-shrink-0 shadow-sm"></span><span className="leading-relaxed">{b}</span></li>))}</ul></div></div>
                </div>
              </div>
              {/* AESTHETIC ACCORDION */}
              {product.aesthetic && (
                <div className={`border rounded-xl overflow-hidden transition-all duration-300 ${open === "aes" ? "border-[#a2d8b2] shadow-md shadow-[#a2d8b2]/20" : "border-gray-100 shadow-sm hover:border-[#a2d8b2]/50"}`}>
                  <button onClick={() => toggle("aes")} className={`w-full flex justify-between items-center px-4 py-3.5 text-left transition-colors ${open === "aes" ? "bg-[#a2d8b2]/10" : "bg-white hover:bg-[#a2d8b2]/5"}`}>
                    ...
                    <div className={`p-1.5 rounded-full transition-all duration-300 ${open === "aes" ? "bg-[#a2d8b2] text-gray-900 rotate-180" : "bg-gray-50 text-gray-400"}`}><ChevronDown className="w-4 h-4" /></div>
                  </button>
                  <div className={`grid transition-all duration-300 ease-in-out ${open === "aes" ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                    <div className="overflow-hidden bg-white"><div className="p-4 pt-3 border-t border-[#a2d8b2]/20 space-y-4 text-sm">
                      <p className="font-bold text-gray-900">{product.aesthetic.title}</p>
                      <p className="text-gray-600 leading-relaxed bg-[#a2d8b2]/10 p-3 rounded-lg border border-[#a2d8b2]/20">{product.aesthetic.description}</p>
                      <p className="font-bold text-gray-900 pt-1">{product.aesthetic.whyChooseTitle}</p>
                      <ul className="space-y-2.5">{product.aesthetic.benefits.map((b: string, i: number) => (<li key={i} className="flex gap-3 items-start text-gray-600"><span className="w-1.5 h-1.5 rounded-full bg-[#a2d8b2] mt-1.5 flex-shrink-0 shadow-sm"></span><span className="leading-relaxed">{b}</span></li>))}</ul>
                      <p className="text-gray-600 leading-relaxed italic border-l-4 border-[#a2d8b2] pl-3 mt-3 py-1">{product.aesthetic.conclusion}</p>
                    </div></div>
                  </div>
                </div>
              )}
